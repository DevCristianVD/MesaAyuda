...
public class DaoUsuario {
    ...
    var sql = "SELECT idusuario, nombre, apellido_paterno, usuario, id_rol FROM tb_usuario WHERE usuario=? AND password_usuario = aes_encrypt(?,?)";
    ...
    try (Connection db = MySqlConexion.getConexion();
        ...
        try (ResultSet rs = ps.executeQuery()) {
            if (rs.next()) {
                ...
                dtoUsuario.setId(rs.getInt("idusuario"));
